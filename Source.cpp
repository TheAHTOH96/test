...
namespace TCP
{
	void server()
	{
		...
		struct sockaddr_in local; //указать локальный или удаленный адрес конечной точки, к которому подключение сокет.
		local.sin_family = AF_INET; //Семейство адреса
		local.sin_port = htons(7500);//Порт IP-адресов
		local.sin_addr.S_un.S_addr = htonl(INADDR_ANY);//.IP-адрес.

		WSADATA wsaData;
		if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) { //инициализирует WinSock
			std::cout << "Failed to load lib." << std::endl;
			return;
		}

		int sock = socket(
			AF_INET,//domain  задает используемый для взаимодействия набор протоколов (вид коммуникационной области), для стека протоколов TCP/IP он должен иметь символьное значение AF_INET.
			SOCK_STREAM,/*Аргумент type задает режим взаимодействия:
SOCK_STREAM - с установлением соединения;
SOCK_DGRAM - без установления соединения.*/
			0);/*Аргумент protocolзадает конкретный протокол транспортного уровня (из нескольких возможных в стеке протоколов). Если этот аргумент задан равным 0, то будет использован протокол "по умолчанию" (TCP для SOCK_STREAM и UDP для SOCK_DGRAM при использовании комплекта протоколов TCP/IP).*/
		if (sock < 0) {
		...
		bind_code = bind(//создания имени сокета
			sock,
		...
		listen_code = listen(sock, 5);//помітити сокет як той, що прослуховує
		if (listen_code) {
			std::cout << "Error during listening" << std::endl;
			return;
		}

		accept_code = accept(sock, nullptr, nullptr);//прийому з'єднання, які чекають у вхідній черзі
		if (accept_code < 0) {
